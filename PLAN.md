...
> Companion study guide: [REFERENCE.md](REFERENCE.md)
>
...
- [ ] Create a virtual environment; `make install` installs `mazegenerator-2.1.0-py3-none-any.whl` plus dev tools (`flake8`, `mypy`, `pytest`).
...
- [ ] Inspect the wheel *as a consumer* (see REFERENCE.md §5): list its contents, read `METADATA`, and probe the public API in a REPL — do **not** modify or vendor its code; it will be re-installed at peer review.
- [ ] Empirically verify the wall-bit encoding (bit 0 = North, bit 1 = East, bit 2 = South, bit 3 = West) and the coordinate order returned by `maze_entry` / `maze_exit` (the README and the implementation disagree — trust your experiment, see REFERENCE.md §5.4).
- [ ] Write a **maze adapter** class: it calls `MazeGenerator(size=(w, h), perfect=False, seed=...)`, catches any generator failure cleanly, and converts the raw wall-encoded `list[list[int]]` into your own internal grid model (never let the rest of the game touch the wheel directly).
...
- [ ] **Pinky (pink)** — chase target = 4 tiles ahead of Pac-Man's facing direction (the ambusher; optionally reproduce the legacy "up = up-and-left" overflow quirk, documented in REFERENCE.md §4.4).
...
- [ ] With the player standing still, each ghost visibly behaves differently (Blinky homes in, Pinky overshoots ahead, Inky flanks, Clyde oscillates).
- [ ] Mode switches are visible (all ghosts flip direction) and frightened ghosts scatter randomly and are eatable.
